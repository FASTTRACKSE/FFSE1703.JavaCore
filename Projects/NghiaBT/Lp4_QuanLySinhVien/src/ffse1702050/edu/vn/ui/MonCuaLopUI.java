...
public class MonCuaLopUI extends JPanel {
	...
	private ArrayList<MonHocCuaLop> arrMoncholop = new ArrayList<MonHocCuaLop>();
	...
	public void timkiemlop() {
		...
		if (cholop.equals("TẤT CẢ")) {
			for (MonHocCuaLop x : arrMoncholop) {
	...
	public void addControls() {
		...
		try {
			...
			ResultSet result = statement.executeQuery("SELECT * FROM lp4_monhoccholop");
			while (result.next()) {
				arrMoncholop.add(
						new MonHocCuaLop(result.getString("MaLop"), result.getString("MaMH"), result.getString("Ten")));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			try {
				...
				ResultSet result = statement.executeQuery(
						"SELECT lp4_class.MaLop,table_monhoc.MaMH,table_monhoc.Ten FROM table_lop INNER JOIN lp4_monhoc WHERE MaLop= '"
								+ lop_SinhVien + "' AND lp4_monhoc.MaMH='" + maMhoc_SinhVien + "'");
				...
				String sql = "INSERT INTO lp4_monhoccholop (MaLop, MaMH,Ten) VALUES (" + "'"
	...
	ActionListener eventDel_lop = new ActionListener() {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			Connection conn = ConnectData.getConnect("localhost", "Lp4", "Lp4", "12345");
			try {
				String sql = "DELETE FROM lp4_monhoccholop WHERE MaLop = '" + (String) maLopcomnoBox.getSelectedItem()
