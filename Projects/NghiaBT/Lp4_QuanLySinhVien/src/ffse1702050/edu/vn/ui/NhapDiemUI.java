...
public class NhapDiemUI extends JPanel {
	...
	public void addControls() {
		...
		try {
			...
			ResultSet result = statement.executeQuery("SELECT * FROM lp4_diem");
			while (result.next()) {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			try {
				if (diem.equals("")) {
					...
					Connection conn = ConnectData.getConnect("localhost", "Lp4", "Lp4", "12345");
					try {
						String sql = "INSERT INTO lp4_diem (MaLop, MaMH,MaSV, Diem) VALUES('" + maLp + "','" + maMH
								+ "','" + sinhVien + "','" + diem + "')";
						Statement statement = conn.createStatement();
			...
			if (nhapDiem1.isEmpty()) {
				...
				if (KT < 2) {
					...
					try {
						Connection conn = ConnectData.getConnect("localhost", "Lp4", "Lp4", "12345");
						try {
							String sql = "UPDATE lp4_diem SET MaLop ='" + chonLopHocDiem + "',MaMH ='"
