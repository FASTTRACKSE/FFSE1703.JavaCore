		...
		for (HinhHoc x : arrHinhHoc) {
			if (x instanceof HinhChuNhat) {
				System.out.printf("|%-20s|%-30s%-30s|%-30s|%-30s| \n", "Hình chữ nhật",
						"Chiều dài: " + ((HinhChuNhat) x).getChieuDai() + ", ",
						"Chiều rộng: " + ((HinhChuNhat) x).getChieuRong() + ", ", x.getChuVi(), x.getDienTich());
			} else if (x instanceof HinhTron) {
				System.out.printf("|%-20s|%-60s|%-30s|%-30s| \n", "Hình tròn: ",
						"Bán kính: " + ((HinhTron) x).getBanKinh(), x.getChuVi(), x.getDienTich());
			} else if (x instanceof HinhTamGiac) {
				System.out.printf("|%-20s|%-20s%-20s%-20s|%-30s|%-30s| \n", "Hình tam giác",
						"Cạnh a: " + ((HinhTamGiac) x).getCanhA() + ", ",
						"Cạnh b: " + ((HinhTamGiac) x).getCanhB() + ", ",
						"Cạnh c: " + ((HinhTamGiac) x).getCanhC() + ", ", x.getChuVi(), x.getDienTich());
			} else if (x instanceof HinhVuong) {
				System.out.printf("|%-20s|%-60s|%-30s|%-30s| \n", "Hình vuông", "Cạnh a: " + ((HinhVuong) x).getCanhA(),
