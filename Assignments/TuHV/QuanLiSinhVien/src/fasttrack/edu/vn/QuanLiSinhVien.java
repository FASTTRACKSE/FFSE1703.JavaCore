...
public class QuanLiSinhVien {
	...
	public static void inDSSV() {
		...
		System.out.println("--------------------------------------------");
		for (i = 0; i < n; i++) {
			System.out.println((i+1)+"   " + ten_SV[i] + "       " +diemLP1[i]+"     "+diemLP2[i] + "     "+ diemDTB[i]);
		}
		myScanner.nextLine();
		System.out.println("Ấn Enter để về menu chính");
		myScanner.nextLine();
	...
	public static void topSV() {
		...
		System.out.println((y+1)+"   " + ten_SV[y] + "       " +diemLP1[y]+"     "+diemLP2[y] + "     "+ diemDTB[y]);

		System.out.println("Học sinh có kết quả học tập thấp nhất là :");
		System.out.println((x+1)+"   " + ten_SV[x] + "       " +diemLP1[x]+"     "+diemLP2[x] + "     "+ diemDTB[x]);
	}
