...
import quanlisinhvien.io.TextFileFactory;
import Quanlysinhvien.model.*;
import java.util.Scanner;
import java.util.ArrayList;
public class Quanlisinhvien {
	public static Scanner myScanner = new Scanner(System.in);
	public static ArrayList<QuanlisinhvienModel> arrSV = new ArrayList<QuanlisinhvienModel>();
	public static int n = 4;
	public static QuanlisinhvienModel[] SV = new QuanlisinhvienModel[4];
	...
	public static void nhapDSSV() {
		...
		for (int j = 0; j < n; j++) {
			...
			arrSV.add(new QuanlisinhvienModel(Name, Date, Lp1, Lp2));
			QuanlisinhvienModel.tinhTongSV();
	...
	public static void inDSSV() {
		...
		System.out.println("Danh sách sinh viên có tổng số là :" + QuanlisinhvienModel.tongSV);
		System.out.println("--------------------------------------------------------------------");
		System.out.println("STT  Họ và tên              Ngày sinh     lp1  lp2   ĐTB  ");
		System.out.println("--------------------------------------------------------------------");
		ArrayList<QuanlisinhvienModel> arrSV = TextFileFactory.docFile("dulieu.txt");
	...
	public static void topSV() {
		...
		System.out.printf("%-23s%-14s%-5s%-5s%-5s\n", arrSV.get(y).getName(), arrSV.get(y).getDate(),
				arrSV.get(y).getLp1(), arrSV.get(y).getLp2(), arrSV.get(y).getTBM());

		System.out.println("Học sinh có kết quả học tập thấp nhất là :");
		System.out.printf("%-23s%-14s%-5s%-5s%-5s\n", arrSV.get(x).getName(), arrSV.get(x).getDate(),
				arrSV.get(x).getLp1(), arrSV.get(x).getLp2(), arrSV.get(x).getTBM());
		myScanner.nextLine();
		System.out.println("Ấn Enter để về menu chính");
		myScanner.nextLine();
	...
	public static void sapxepTBM() {
		QuanlisinhvienModel[] temp = new QuanlisinhvienModel[arrSV.size()];
	...
	public static void doitenSV() {
		...
		for (QuanlisinhvienModel x : arrSV) {
	...
	public static void xoaSV() {
		...
		for (int i = arrSV.size() - 1; i > -1; i++) {
			if (arrSV.get(i).getName().equals(xoaTen)) {
				arrSV.remove(arrSV.get(i));
				QuanlisinhvienModel.tongSV -= 1;
	...
	public static void showMyMenu() {
		while (true) {
			...
			System.out.println("|7. Xóa tên học sinh                     |");
